...
class MainMenuBar(QMenuBar):
    ...
    def connect_triggers(self, driver):
        # region File Menu ============================================================
        self.open_library_action.triggered.connect(driver.open_library_from_dialog)
        self.save_library_backup_action.triggered.connect(
            lambda: driver.call_if_library_open(driver.backup_library)
        )
        self.settings_action.triggered.connect(driver.open_settings_modal)
        self.open_on_start_action.setChecked(driver.settings.open_last_loaded_on_startup)
        self.open_on_start_action.triggered.connect(driver.set_open_last_loaded_on_startup)
        self.refresh_dir_action.triggered.connect(
            lambda: driver.call_if_library_open(driver.add_new_files_callback)
        )
        self.close_library_action.triggered.connect(driver.close_library)
        # endregion

        # region Edit Menu ============================================================
        self.new_tag_action.triggered.connect(driver.add_tag_action_callback)
        self.select_all_action.triggered.connect(driver.select_all_callback)
        self.select_inverse_action.triggered.connect(driver.select_inverse_callback)
        self.clear_select_action.triggered.connect(driver.clear_select_callback)
        self.copy_fields_action.triggered.connect(driver.copy_fields_action_callback)
        self.paste_fields_action.triggered.connect(driver.paste_fields_action_callback)
        self.add_tag_to_selected_action.triggered.connect(driver.add_tag_modal.show)
        self.delete_file_action.triggered.connect(lambda f="": driver.delete_files_callback(f))
        self.tag_manager_action.triggered.connect(driver.tag_manager_panel.show)
        self.color_manager_action.triggered.connect(driver.color_manager_panel.show)
        # endregion

        # region View Menu ============================================================
        self.library_info_action.triggered.connect(driver.create_library_info_window)
        self.show_filenames_action.triggered.connect(driver.show_grid_filenames)
        self.show_filenames_action.setChecked(driver.settings.show_filenames_in_grid)
        self.decrease_thumbnail_size_action.triggered.connect(driver.decrease_thumbnail_size)
        self.increase_thumbnail_size_action.triggered.connect(driver.increase_thumbnail_size)
        # endregion

        # region Tools Menu ===========================================================
        self.fix_unlinked_entries_action.triggered.connect(driver.create_fix_unlinked_entries_modal)
        self.fix_ignored_entries_action.triggered.connect(driver.create_ignored_entries_modal)
        self.fix_dupe_files_action.triggered.connect(driver.create_dupe_files_modal)
        # TODO: Move this to a settings screen.
        self.clear_thumb_cache_action.triggered.connect(lambda: driver.cache_manager.clear_cache())
        # endregion

        # region Macros Menu ==========================================================
        self.folders_to_tags_action.triggered.connect(driver.create_folders_tags_modal)
        # endregion

        # region Help Menu ============================================================
        self.about_action.triggered.connect(driver.create_about_modal)
        # endregion
    ...
    def set_action_visibility(self, state: bool):
        self.save_library_backup_action.setEnabled(state)
        self.close_library_action.setEnabled(state)
        self.refresh_dir_action.setEnabled(state)
        self.tag_manager_action.setEnabled(state)
        self.color_manager_action.setEnabled(state)
        self.ignore_modal_action.setEnabled(state)
        self.new_tag_action.setEnabled(state)
        self.fix_unlinked_entries_action.setEnabled(state)
        self.fix_ignored_entries_action.setEnabled(state)
        self.fix_dupe_files_action.setEnabled(state)
        self.clear_thumb_cache_action.setEnabled(state)
        self.folders_to_tags_action.setEnabled(state)
        self.library_info_action.setEnabled(state)

    def set_clipboard_menu_visibility(self, num_selected: int, has_copy_buffer: bool):
        if num_selected == 1:
            self.copy_fields_action.setEnabled(True)
        else:
            self.copy_fields_action.setEnabled(False)
        if num_selected > 0 and has_copy_buffer:
            self.paste_fields_action.setEnabled(True)
        else:
            self.paste_fields_action.setEnabled(False)

    def set_select_actions_visibility(self, num_selected: int, has_frame_content: bool):
